...
// v11.4.1 — 본대 외 차편에 차수별 전체보기·사진·인쇄 // 인쇄 자동 크기(균등 분면·한 장 채움), 불참·환불은 차수 없이 저장, 전도 명단 차수 열 // '차수 해제=그 차수 불참' 원칙(표식·이력·도움말) // 차수 명단: 그 차수 교통편만·비고 제거 // 숙소 정원 소실 근본 수정(rev 분리 + 정원 보존 병합) // 홈 상단 2줄 재배치(등록·전도 / 구원·침례·접수현황·기도명단), 구원 명단 시트 // 사진 최대화질(긴 명단 여러 장 분할)·한 번에 공유, 인쇄 한 장 맞춤, 전도 사진 버튼, 이력 병합 보존 // 숙소 탭 신설(칩·정원·배정·미등록 인원), 차수 미정 지원, 숙소는 본 차수 전용, 사진 최대 화질
const CACHE_VERSION = 'retreat-sync-v11.4.1';
const CORE_ASSETS = [
  './',
  './index.html',
  './manifest.json',
